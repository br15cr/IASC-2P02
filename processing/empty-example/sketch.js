...
function draw() {
  ...
  pieChart(250, data, colors);
	printTopics(topics,colors);
...
function printTopics(topics,colors){
	textSize(20);
...
function drawBackground(docs){
	...
	for(let i = 0; i < docs.length;i++){
		...
		noStroke();
		text(docs[i],20,ypos+50);
		if(mouseY > ypos && mouseY <= ypos+gap){
			hover = i;
		}
	}	
	noStroke();
